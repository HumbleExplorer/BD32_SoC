...
#   ARR = 255 => period = 256 timer-ticks per PWM cycle
#   PSC = 1   => prescaler div = 2 (PSC=0 is reserved)
#   Each PWM period: poll SR[0], update CCR1, change by step (+1 or -1)
#   CCR range: 0 (0% duty, LED off) ~ 255 (100% duty, LED fully on)
#
...
_start:
    # ----------------------------------------------------------------
    # Step 0: Print welcome text from DTCM (0x00020000 ~ 0x00026000)
    # ----------------------------------------------------------------
...
print_welcome:
    ...
    # Set ARR = 255 (up-counting: overflow at cnt==255, period=256 ticks)
    li t1, 255
    sw t1, TIMx_ARR(t0)
...
breathing_loop:
    # ---- Wait for one PWM period end (poll SR overflow flag) ----
wait_overflow:
    lw t2, TIMx_SR(s0)     # Read SR
    andi t2, t2, 0x01       # Check bit[0] = timer_int_of_pend
    beqz t2, wait_overflow  # Not set yet -> keep polling

    # Clear the overflow flag (write 1 to SR[0])
    li t2, 0x01
    sw t2, TIMx_SR(s0)

    # ---- Update CCR1 for next period ----
    # Update direction:
    #   if increasing (s2==0) and CCR1 == 255 -> switch to decreasing
...
check_top:
    bne s1, t3, do_increase # CCR1 != 255 -> keep increasing
