...
mkdir "%LOGS%" 2>nul
cd /d "%~dp0"
echo vlog-start >> "%LOGS%\rtthread_sim_mark.txt"
"%MODELSIM_PATH%\vsim.exe" -batch -do "do rtthread_sim.do" > "%LOGS%\rtthread_sim_out.txt" 2>&1
echo vsim-exit-%errorlevel% >> "%LOGS%\rtthread_sim_mark.txt"
echo done: logs\rtthread_sim_out.txt
@endlocal
