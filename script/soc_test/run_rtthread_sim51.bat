@echo off
rem ============================================================================
rem BD32 RT-Thread v5.1.0 demo SoC simulation (tb_soc_top, DIRECT_LOAD)
...
mkdir "%LOGS%" 2>nul
cd /d "%~dp0"
echo vlog-start >> "%LOGS%\rtthread_sim51_mark.txt"
"%MODELSIM_PATH%\vsim.exe" -batch -do "do rtthread_sim51.do" > "%LOGS%\rtthread_sim51_out.txt" 2>&1
echo vsim-exit-%errorlevel% >> "%LOGS%\rtthread_sim51_mark.txt"
echo done: logs\rtthread_sim51_out.txt
@endlocal
