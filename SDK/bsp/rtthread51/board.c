/*
 * BD32 RT-Thread v5.1.0 BSP —— 板级初始化 / tick / 控制台 / trap 分发
 *
 * 中断架构（官方 CH32 模式）：
 *   7/11 号（定时器/外部）→ 轻量入口（rt_trap.S）：只存 caller 寄存器，
 *   处理完 mret；需要切换时 rt_hw_context_switch_interrupt 置 flag 并写
 *   CLINT_MSIP 触发软件中断。
 *   3 号软件中断 → SW_handler（官方 interrupt_gcc.S）：全量保存 + flag 切换，
 *   延迟切换在此完成（PendSV 模式）。
 */
#include <rthw.h>
#include <rtthread.h>
...
void serr_handler(uint32_t mcause, uint32_t mepc)
{
    ...
    uart_puts("\r\n[SERR] thread=");
    if (self) uart_puts(self->parent.name);
    if (self) {
        uart_puts(" tcb_sp=0x");
...
/* ===================================================================
 * 统一 trap 分发（trap_entry 调用，参数来自汇编）
 *   a0 = mcause, a1 = mepc, a2 = 栈帧指针
 * =================================================================== */
/* ===================================================================
