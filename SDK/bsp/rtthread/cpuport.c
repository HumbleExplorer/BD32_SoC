/*
 * BD32 RT-Thread v3.1.5 RISC-V libcpu（官方 lts-v3.1.x unify port）
 *
 * 来源：RT-Thread 官方 lts-v3.1.x 分支 libcpu/risc-v/common/cpuport.c
 * （随 yuheng-riscv-soc 参考工程携带），Apache-2.0。
 *
 * BD32 适配（延迟切换 / PendSV 方案，与 v5.1 行为一致）：
 *   1. 新增 weak rt_trigger_software_interrupt()（board.c 覆盖为写
 *      CLINT_MSIP=1 触发机器软件中断）。
 *   2. 新增 weak rt_hw_do_after_save_above()（board.c 覆盖为清 MSIP）。
 *   3. rt_hw_context_switch_interrupt 置 flag 后调用
 *      rt_trigger_software_interrupt()，让 3 号软件中断执行真正的全量
 *      上下文切换（与官方 v5.1 相同）。
 */
...
/* BD32：软件中断触发/保存后处理 weak 桩（board.c 强定义覆盖） */
...
void rt_hw_context_switch_interrupt(rt_ubase_t from, rt_ubase_t to)
{
    ...
    /* BD32：触发机器软件中断，由 SW_handler 执行延迟切换 */
