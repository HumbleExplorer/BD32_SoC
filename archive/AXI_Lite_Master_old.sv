...
`timescale 1ns / 1ps
// =============================================================================
// AXI_Lite_Master - AXI4-Lite 主设备接口
// =============================================================================
// 功能：
...
//
// 时序：
//   写：IDLE → WAIT_AW(WADDR握手) → WAIT_W(WDATA握手) → WAIT_B(BRESP) → IDLE
//   读：IDLE → WAIT_AR(RADDR握手) → WAIT_R(RDATA) → IDLE
// =============================================================================
...
module AXI_Lite_Master #(
    ...
    always_comb begin
        ...
        case (state)
            IDLE: begin
                if (req_valid)
                    next_state = req_write ? WAIT_AW : WAIT_AR;
            end
    ...
    assign m_awvalid = (state == WAIT_AW);

    // 写数据
    assign m_wdata   = wdata_reg;
    assign m_wstrb   = wstrb_reg;
    assign m_wvalid  = (state == WAIT_W);
    ...
    assign m_arvalid = (state == WAIT_AR);
